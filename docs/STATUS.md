...
|---|---|---|---|
| 2026-07-16 | Claude Code (revisión) | **Revisión configuración Perplexity — sender SMTP incorrecto.** Confirmado: el bot usó `onboarding@resend.dev` como Sender email en Authentication → SMTP Settings. Ese es el dominio sandbox de Resend — no apto para producción (emails van a spam o bloqueados). Fix inmediato recomendado: cambiar Sender email a `noreply@soymanada.com`, que ya tiene dominio verificado en Resend (mismo que usa el Edge Function notify-submission). Esto mantiene consistencia temporal hasta que R8 se ejecute (migrar a dominio ODAC antes de septiembre). Instrucciones para el bot al pie de esta entrada. | Bot actualiza Sender email SMTP |
